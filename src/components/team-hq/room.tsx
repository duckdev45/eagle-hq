  ...
  useEffect(() => {
    team.forEach((m) => {
      ...
      if (el && p) {
        ...
        el.style.top = `${p.y}px`
        el.style.zIndex = String(zOf(p.y))
      }
    })
  })
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        acc += dt
        if (acc >= FRAME_MS) { acc = 0; frame = (frame + 1) % frames }
      } else {
        frame = 0
        acc = 0
      }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ left: PUNCH_CLOCK.x, top: PUNCH_CLOCK.y, position: 'absolute', zIndex: zOf(PUNCH_CLOCK.y + PUNCH_CLOCK.h - SPRITE_H) }}
      ...
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src='/items/office_items/punch_clock.png' alt='punch clock' style={{ width: PUNCH_CLOCK.w, height: PUNCH_CLOCK.h }} draggable={false} />
      ...
      {team.map((m) => (
        <MemberSprite
          ...
          setContainerRef={(el) => { containerRefs.current[m.id] = el }}
          setImgRef={(el) => { imgRefs.current[m.id] = el }}
        />
      ))}
    </div>
  )
