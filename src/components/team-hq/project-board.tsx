import type { Lang, Project } from '@/types/team'
import type { EpicHealth, HealthLight } from '@/types/jira'
import { HQ_I18N } from '@/data/i18n'

import { PixelBoard } from './pixel/pixel-board'

interface ProjectBoardProps {
  projects: Project[]
  epics?: EpicHealth[] | null
  onPickEpic?: (key: string) => void
  onPick: (id: string) => void
  onAdd: () => void
  lang: Lang
  open: boolean
  onToggle: () => void
}

const LAMP_TXT: Record<string, string> = { g: '🟢 ', y: '🟡 ', r: '🔴 ' }
const LIGHT_CLS: Record<HealthLight, 'g' | 'y' | 'r'> = { green: 'g', yellow: 'y', red: 'r' }

export function ProjectBoard({ projects, epics, onPickEpic, onPick, onAdd, lang, open, onToggle }: ProjectBoardProps) {
  const t = HQ_I18N[lang]
  const zh = lang === 'zh'
  const jiraMode = !!epics && epics.length > 0
  const reds = jiraMode ? epics.filter((e) => e.light === 'red') : projects.filter((p) => p.light === 'r')

  const addButton = jiraMode
    ? <span className='jira-src'>JIRA</span>
    : <button className='add-btn' onClick={onAdd} title={t.addProj}>＋</button>

  return (
    <PixelBoard
      title='PROJECTS'
      zhTitle={t.projects}
      headerRight={addButton}
      className='lights'
      open={open}
      onToggle={onToggle}
    >
      {reds.length > 0 && (
        <div className='r-banner'>
          <b>● {reds.length}</b>
          {zh
            ? <> {t.needHead} PM {t.needTail}</>
            : <> need PM&apos;s call</>}
        </div>
      )}

      {jiraMode
        ? epics.map((e) => {
            const cls = LIGHT_CLS[e.light]
            return (
              <div key={e.key} className={`proj ${cls}`} onClick={() => onPickEpic?.(e.key)}>
                <span className='lamp' />
                <div className='pinfo'>
                  <div className='pname'>{e.summary}</div>
                  <div className='pstat'>
                    {LAMP_TXT[cls]}{e.key} · {e.counts.done}/{e.counts.total} {zh ? '完成' : 'done'}
                  </div>
                </div>
                <div className='pdue'>{e.dueDate ?? ''}</div>
                {e.light !== 'green' && e.reasons.length > 0 && (
                  <div className='note'>{e.reasons.join('、')}</div>
                )}
              </div>
            )
          })
        : projects.map((p) => (
            <div key={p.id} className={`proj ${p.light}`} onClick={() => onPick(p.id)}>
              <span className='lamp' />
              <div className='pinfo'>
                <div className='pname'>{p.id}</div>
                <div className='pstat'>
                  {LAMP_TXT[p.light]}{zh ? p.stat : (p.stat_en || p.stat)}
                </div>
              </div>
              <div className='pdue'>{p.due}</div>
              {p.light !== 'g' && (
                <div className='note'>{zh ? p.note : (p.note_en || p.note)}</div>
              )}
            </div>
          ))}
    </PixelBoard>
  )
}
